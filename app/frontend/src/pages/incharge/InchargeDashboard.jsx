...
export default function InchargeDashboard() {
  ...
  if (!lab) {
    return (
      <Layout>
        <PageHeader title="Lab Incharge Dashboard" />
        <EmptyState title="No lab assigned" hint="Ask an admin to assign you as the incharge of a lab." />
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard testId="incharge-stat-equipment" label="Equipment Items" value={equipment.length} color="blue" />
        <StatCard testId="incharge-stat-units" label="Total Units" value={totalUnits} hint={`${issuedUnits} issued`} color="green" />
        <StatCard testId="incharge-stat-pending" label="Pending Requests" value={pending} color="amber" />
        <StatCard testId="incharge-stat-issued" label="Currently Issued" value={issued} color="red" />
      </div>
